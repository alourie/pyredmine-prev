...
#        custom_fields = self.root.find('custom_fields')
#        if len(custom_fields) > 0:
#            for field in custom_fields:
#                if field.attrib and "name" in field.attrib.keys():
...
#            self.tracker[ tracker.attrib['name'] ] = tracker.attrib['id']
    ...
    def newIssue(self, **data ):
        ...
        if not 'subject' in data.keys():
            raise TypeError('Subject cannot be blank.  Use subject=str')
...
class Redmine:
    ...
    def post(self, page, objXML, parms=None ):
        ...
        if self.readonlytest:
            ...
            return self.open( page=page, parms=parms, dict=objXML )
    ...
    def newIssueFromDict(self, dict ):
        ...
        #print "xmlStr is \n%s\n" % xmlStr
        newIssue = self.Issue( self.post( 'issues.xml', dict ) )
        return newIssue
